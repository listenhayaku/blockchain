...
class Blockchain():
    ...
    def others_ledge_verify(self):
        ...
        try:    #這裡會引發例外也是因為沒有收到others_ledge_set
            ...
            #比對prevhash
            '''
            for i in range(others_chain) - 1:   #i:0~8
                if(i == 0): 
                    if(others_chain[0].prev_hash != self.chain[others_chain[0].id-1].get_blockhash()):  #其他帳本跟自己的接不上
                        print("(debug)[others_ledge_verify]test verify failed!")
                        return False
                else:
                    if(others_chain[i+1].prev_hash != others_chain[i].get_blockhash())
            '''
    ...
    def others_ledge_replace(self): #驗證都通過才可以來這裡
        try:
            ...
            #for i in range(others_chain[0].id,len(self.chain)):
                #print("(debug)[others_ledge_replace]i:",i)
                #self.chain.pop(len(self.chain)-1)
            #self.print_blockchain(debug="after pop")
            self.append_blockchain(others_chain)
            #self.print_blockchain(debug="after replace")
            #delete blockqueue
    ...
    def write_log_block(self,block,type = "droppped",debug = ""):
        ...
        if self.port == 5000:
            ...
            f = open("{path}5003/verify_{type}.log".format(path = path,type = type),'a')
        '''    
        f = open("{path}{port}/verify_{type}.log".format(path = path,port = self.port,type = type),'a')
